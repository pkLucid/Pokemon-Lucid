...
static s32 (*const sBattleAiFuncTable[])(u32, u32, u32, s32) =
{
    ...
    [3] = NULL,                      // Unused
    [4] = NULL,                      // Unused
    [5] = AI_Roaming,                // AI_FLAG_ROAMING
    [6] = AI_Safari,                 // AI_FLAG_SAFARI
    [7] = AI_FirstBattle,            // AI_FLAG_FIRST_BATTLE
};
...
void Ai_UpdateSwitchInData(u32 battler)
{
    ...
    SetBattlerAiMovesData(aiData, battler, gBattlersCount, AI_GetWeather(aiData));
}
...
void SetBattlerAiData(u32 battler, struct AiLogicData *aiData)
{
    ...
    if (IsStatBoostingMove(battler, gLastResultingMoves[battler]))
        gBattleStruct->usedSetUpMoveBefore |= (1u << battler);
}

static inline bool32 PlayerSideHasMove(u32 move)
{
    return HasMove(B_POSITION_PLAYER_LEFT, move) || HasMove(B_POSITION_PLAYER_RIGHT, move);
}

static inline bool32 ShouldCalcSpreadMoveDamage(u32 battler)
{
    if (RandomPercentage(RNG_SPREAD_MOVE_CALC, 80))
        return FALSE;

    if (gBattleStruct->playerUsedWideGuard && PlayerSideHasMove(MOVE_WIDE_GUARD))
        return TRUE;
...
static void SetBattlerAiMovesData(struct AiLogicData *aiData, u32 battlerAtk, u32 battlersCount, u32 weather)
{
    ...
    u32 calcSpreadMoveDamage = ShouldCalcSpreadMoveDamage(battlerAtk);
    ...
    for (u32 battlerDef = 0; battlerDef < battlersCount; battlerDef++)
    {
        ...
        for (u32 moveIndex = 0; moveIndex < MAX_MON_MOVES; moveIndex++)
        {
            ...
            if (move != 0 && move != 0xFFFF && !(aiData->moveLimitations[battlerAtk] & (1u << moveIndex)))
                dmg = AI_CalcDamage(move, battlerAtk, battlerDef, &effectiveness, TRUE, weather, calcSpreadMoveDamage);
    ...
    for (i = 0; i < MAX_BATTLERS_COUNT; i++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (gMovesInfo[moves[j]].target == MOVE_TARGET_USER
                     || gMovesInfo[moves[j]].target == MOVE_TARGET_OPPONENTS_FIELD
                     || gMovesInfo[moves[j]].target == MOVE_TARGET_ALL_BATTLERS)
                        continue;
...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (aiData->effectiveness[battlerAtk][battlerDef][AI_THINKING_STRUCT->movesetIndex] == AI_EFFECTIVENESS_x0 && moveEffect != EFFECT_FUTURE_SIGHT)
    ...
    switch (moveEffect)
    {
        ...
        case EFFECT_FILLET_AWAY:
            if (aiData->abilities[battlerAtk] == ABILITY_CONTRARY)
                ADJUST_SCORE(FAILS_OR_BAD);
            else if (NoOfHitsToFaintBattler(battlerDef, battlerAtk) == 1)
                ADJUST_SCORE(FAILS_OR_BAD);
            break;
        ...
        case EFFECT_HELPING_HAND:
            if (!isDoubleBattle || !IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
                ADJUST_SCORE(FAILS_OR_BAD);
            else if (battlerAtk > BATTLE_PARTNER(battlerAtk) && HasMoveEffect(BATTLE_PARTNER(battlerAtk), moveEffect))
                ADJUST_SCORE(FAILS_OR_BAD);
            // else if (aiData->partnerMove != MOVE_NONE && IsBattleMoveStatus(aiData->partnerMove))
            //     ADJUST_SCORE(FAILS_OR_BAD);
        ...
        case EFFECT_METRONOME:
            break;
        case EFFECT_REST:
            if (!CanBeSlept(battlerAtk, aiData->abilities[battlerAtk], TRUE))
                ADJUST_SCORE(FAILS_OR_BAD);
            break;
        case EFFECT_SLEEP:
        case EFFECT_YAWN:
        ...
        case EFFECT_LASER_FOCUS:
            if (gStatuses3[battlerAtk] & STATUS3_LASER_FOCUS)
                ADJUST_SCORE(FAILS_OR_BAD);
            else if (HasBattlerCritProtectingAbility(aiData->abilities[battlerDef]))
                ADJUST_SCORE(FAILS_OR_BAD);
            break;
        case EFFECT_SKETCH:
        ...
        case EFFECT_DEFOG:
            ...
            if (isDoubleBattle)
            {
                if (IsHazardMoveEffect(gMovesInfo[aiData->partnerMove].effect) // partner is going to set up hazards
                  && AI_IsFaster(BATTLE_PARTNER(battlerAtk), battlerAtk)) // partner is going to set up before the potential Defog
                {
                    ADJUST_SCORE(FAILS_OR_BAD);
        ...
        case EFFECT_JUNGLE_HEALING:
           if (IsBattlerAtMaxHp(battlerAtk)
            && IsBattlerAtMaxHp(BATTLE_PARTNER(battlerAtk))
        ...
        case EFFECT_UPPER_HAND:
            if (predictedMove == MOVE_NONE || IsBattleMoveStatus(predictedMove) || !AI_IsFaster(battlerAtk, battlerDef) || GetBattleMovePriority(battlerDef, predictedMove) < 1 || GetBattleMovePriority(battlerDef, predictedMove) > 3) // Opponent going first or not using priority move
                ADJUST_SCORE(FAILS_OR_BAD);
            break;
...
static s32 AI_CalcPartnerScore(u32 battler, u32 battlerPartner, u32 move)
{
    ...
    if (partnerHoldEffect == HOLD_EFFECT_WEAKNESS_POLICY
     ...
     && AI_IsFaster(battler, battlerPartner)
    ...
    case EFFECT_SWAGGER:
        {
            u32 opposingFieldHasUnnerve = (aiData->abilities[BATTLE_OPPOSITE(battler)] == ABILITY_UNNERVE) || (aiData->abilities[BATTLE_OPPOSITE(battlerPartner)] == ABILITY_UNNERVE);
            u32 partnerCanCureConfusion = !opposingFieldHasUnnerve && (partnerHoldEffect == HOLD_EFFECT_CURE_STATUS || partnerHoldEffect == HOLD_EFFECT_CURE_CONFUSION);
            if (!partnerCanCureConfusion && partnerAbility != ABILITY_OWN_TEMPO)
                return NO_INCREASE;
            if (gBattleMons[battlerPartner].statStages[STAT_ATK] <= DEFAULT_STAT_STAGE && IsBattlerOrPartnerFirstTurn(battler, battlerPartner))
                return BEST_EFFECT;
        }
        break;
    }
...
static u32 AI_CalcEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    {
    case EFFECT_ABSORB:
        if (AI_ShouldAbsorb(battlerAtk, battlerDef, move))
            ADJUST_SCORE(BEST_EFFECT);
        break;
    case EFFECT_FUTURE_SIGHT:
        if (AI_TryFutureSight(battlerAtk, battlerDef))
            ADJUST_SCORE(BEST_EFFECT);
        break;
    case EFFECT_DESTINY_BOND:
        if (CanBattlerFaintTarget(battlerDef, battlerAtk)
         && (AI_IsFaster(battlerAtk, battlerDef) || AI_DATA->holdEffects[battlerAtk] == HOLD_EFFECT_QUICK_CLAW)
         && Random() & 1) //RandomPercentage(RNG_NONE, 70))
            ADJUST_SCORE(BEST_EFFECT);
        break;
    case EFFECT_SLEEP:
    case EFFECT_YAWN:
    ...
    case EFFECT_TOXIC_THREAD:
        if (AI_ShouldPoison(battlerAtk, battlerDef))
            ADJUST_SCORE(BEST_EFFECT);
        score += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPEED, FALSE);
    ...
    case EFFECT_SHED_TAIL:
    case EFFECT_SUBSTITUTE:
        if (AI_ShouldSubstitute(battlerAtk, battlerDef, move))
            ADJUST_SCORE(BEST_EFFECT);
        break;
    case EFFECT_FOLLOW_ME:
    ...
    case EFFECT_CURSE:
        ...
        {
            score += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, FALSE);
            score += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, FALSE);
            score = (score > BEST_EFFECT) ? BEST_EFFECT : score;
        }
        break;
    case EFFECT_PROTECT:
    ...
    case EFFECT_STOCKPILE:
        if (HasMoveEffect(battlerAtk, EFFECT_SWALLOW) || HasMoveEffect(battlerAtk, EFFECT_SPIT_UP))
            ADJUST_SCORE(GOOD_EFFECT);
        score += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, FALSE);
        score += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, FALSE);
        score = (score > BEST_EFFECT) ? BEST_EFFECT : score;
        break;
    case EFFECT_ATTRACT:
    ...
    case EFFECT_DEFOG:
        ...
        else if (!(gSideStatuses[GetBattlerSide(battlerDef)] & SIDE_STATUS_SPIKES)) //Don't blow away hazards if you set them up
        {
            ...
            {
                if (IsHazardMoveEffect(gMovesInfo[aiData->partnerMove].effect) // Partner is going to set up hazards
                    && !AI_IsFaster(battlerAtk, BATTLE_PARTNER(battlerAtk))) // Partner going first
                    break; // Don't use Defog if partner is going to set up hazards
            }
    ...
    case EFFECT_EMBARGO:
        ...
        break;
    case EFFECT_POWDER:
        if (predictedMove != MOVE_NONE && !IsBattleMoveStatus(predictedMove) && gMovesInfo[predictedMove].type == TYPE_FIRE)
            ADJUST_SCORE(GOOD_EFFECT);
        break;
    case EFFECT_TELEKINESIS:
        if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, FALSE, aiData->abilities[battlerAtk], aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk], aiData->holdEffects[battlerDef])
        ...
        break;
    case EFFECT_HEAL_BLOCK:
        if (AI_IsFaster(battlerAtk, battlerDef) && predictedMove != MOVE_NONE && IsHealingMove(predictedMove))
    ...
    case EFFECT_FAIRY_LOCK:
        ...
        break;
    case EFFECT_QUASH:
        if (isDoubleBattle && !AI_IsFaster(BATTLE_PARTNER(battlerAtk), battlerDef))
    ...
    case EFFECT_MAGNET_RISE:
        ...
        break;
    case EFFECT_CAMOUFLAGE:
        if (predictedMove != MOVE_NONE && AI_IsFaster(battlerAtk, battlerDef)
...
static u32 AI_CalcFaintScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    {
        if (AI_IsFaster(battlerAtk, battlerDef) || GetBattleMovePriority(battlerAtk, move) > 0)
            faintScore += GOOD_EFFECT;
...
static s32 AI_CalcSetUpEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (gBattleStruct->usedSetUpMoveBefore & (1u << battlerAtk)
    ...
    case EFFECT_HIT:
        ...
        case MOVE_FLEUR_CANNON:
            if (AI_DATA->abilities[battlerAtk] == ABILITY_CONTRARY)
                tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK, TRUE);
            break;
        case MOVE_SUPERPOWER:
            if (AI_DATA->abilities[battlerAtk] == ABILITY_CONTRARY)
                tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, TRUE);
            break;
        }
        break;
    case EFFECT_COSMIC_POWER:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, FALSE);
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, FALSE);
        break;
    case EFFECT_BULK_UP:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, FALSE);
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, FALSE);
        break;
    case EFFECT_CALM_MIND:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK, FALSE);
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, FALSE);
        break;
    case EFFECT_GEOMANCY:
        if (AI_DATA->holdEffects[battlerAtk] != HOLD_EFFECT_POWER_HERB)
            break;
    case EFFECT_QUIVER_DANCE:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPEED, FALSE);
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK, FALSE);
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, FALSE);
        break;
    case EFFECT_VICTORY_DANCE:
    ...
    case EFFECT_ATTACK_UP_USER_ALLY:
        ...
        break;
    case EFFECT_ATTACK_UP_2:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK_2, FALSE);
        break;
    case EFFECT_DEFENSE_UP:
    case EFFECT_DEFENSE_UP_3:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_DEF, FALSE);
    ...
    case EFFECT_SPECIAL_ATTACK_UP_2:
    case EFFECT_SPECIAL_ATTACK_UP_3:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPATK_2, FALSE);
        break;
    case EFFECT_SPECIAL_DEFENSE_UP:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF, FALSE);
        break;
    case EFFECT_SPECIAL_DEFENSE_UP_2:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_SPDEF_2, FALSE);
        break;
    case EFFECT_ACCURACY_UP:
    case EFFECT_ACCURACY_UP_2:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ACC, FALSE);
        break;
    case EFFECT_EVASION_UP:
    case EFFECT_EVASION_UP_2:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_EVASION, FALSE);
        break;
    case EFFECT_ATTACK_ACCURACY_UP:
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ATK, FALSE);
        tempScore += IncreaseStatUpScore(battlerAtk, battlerDef, STAT_CHANGE_ACC, FALSE);
...
static inline bool32 AI_TryToFaintWithCrit(u32 battlerAtk, u32 battlerDef, u32 moveIndex)
{
    u16 *moves = GetMovesArray(battlerAtk);

    if (!gMovesInfo[moves[moveIndex]].criticalHitStage)
        return FALSE;
    if (Random() & 1)
        return FALSE;

    u32 criticalHitDmg = AI_DATA->simulatedDmg[battlerAtk][battlerDef][moveIndex].criticalHit;
    if (criticalHitDmg == 0)
        return FALSE;

    if (GetNoOfHitsToKOBattlerDmg(criticalHitDmg, battlerDef) == 1)
    {
...
static s32 AI_CheckViability(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        u32 partnerScore = AI_CalcPartnerScore(battlerAtk, battlerDef, move);
        if (partnerScore == 0)
            return NO_INCREASE;
        else
            RETURN_SCORE(partnerScore);
    }
    ...
    if (IsDoubleBattle())
    {
        ...
        {
        case EFFECT_PLEDGE:
            if (AI_ShouldUsePledgeMove(battlerAtk, battlerDef, move))
            {
                if (gBattleStruct->aiComboAttackState == COMBO_INITIAL_STATE)
                    gBattleStruct->aiComboAttackState = COMBO_FIRST_BATTLER_SCORE_INCREASE;
                ADJUST_SCORE(BEST_EFFECT);
            }
            break;
        case EFFECT_ROUND:
            if (AI_ShouldUseRound(battlerAtk))
            {
                if (gBattleStruct->aiComboAttackState == COMBO_INITIAL_STATE)
                    gBattleStruct->aiComboAttackState = COMBO_FIRST_BATTLER_SCORE_INCREASE;
                ADJUST_SCORE(BEST_EFFECT);
    ...
    if (GetMovePower(move) != 0)
    {
        ...
        else if (AI_TryToFaintWithCrit(battlerAtk, battlerDef, moveIndex))
            score += GOOD_EFFECT;
    ...
    ADJUST_SCORE(AI_CalcEffectScore(battlerAtk, battlerDef, move));
    return score;
}
